...
One of the most frquently used tools for RNA-seq differential analysis is DESeq2. It uses a generalized linear model (GLM) framework with a negative binomial distribution to model RNA-seq count data, accounting for both the mean-variance relationship and overdispersion.<br>
The GLM includes a design matrix to specify the experimental design, and an empirical Bayes shrinkage estimator is used to improve the stability and accuracy of the dispersion estimates. This modeling approach allows for accurate and reliable identification of differentially expressed genes in RNA-seq data.

### Exercise 3.1 - Differential expression on a single dataset
Lets start with the basic analysis of one of our datasets! 
We'll see how to handle batch effects in the next chapter and then use both datasets. We'll settle for GSE68799, as the difference between Normal and Tumor samplees was much clearer in the PCA, making it likely the "easier" dataset.
```{r, echo=F}
# solution
...
```{r, echo=F}
# Exercise 3.1.1: Create a DESeqDataSet object
