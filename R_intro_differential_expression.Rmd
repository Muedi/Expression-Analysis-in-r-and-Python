...
The thread I found these images is from crossvalidated, a sub-site of stackexchange, where one can often find answers to questions regarding statistics and machine learning. I recommend to check out the thread, it has a number of answers, ranging from very mathematical to much mnore intuitive explanations (as hopefully given above :D)
...
### And a scatterplot? 
A scatterplot is the most simple plot you can come up with, it plots two numerical variables against each other leading to a point cloud. We'll plot the first two principal components against each other, as we'll see in a minute. 


### Exercise 2.1 <br> computing PCA
load the stats package and use the **prcomp()** function to conmpute the principal components of our expression data.
Tipp: there is also **rownames_to_column()**
```{r message=FALSE, warning=FALSE, include=FALSE}
df_pca <- prcomp(counts) 
df_plot <- df_pca$x %>% as_tibble()
