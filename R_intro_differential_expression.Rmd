...
```
What we have at our hands was formatted to be used with ML tools: <br>
...
However, in most other applications in R, like statistical functions and also the bioinformatic packages we'll be using, the opposite is expected: 
...
<br>Tipp: you can *eselect* a column, by using **-** infront of the column name.
...
# Exercise 1.3 b) make count matrix
...
a. Use the **prcomp()** function to compute the principal components of our expression data and save it in a variable.
...
Feel free to use the internet, there are a plethora of tutorials online. There is also a lot of information in the vignettes, try ```?model.matrix``` and ```limmaUsersGuide()``` for example.
<br>Remember, that you DO NOT need to use voom as we already have normalized data.
```{r}
...
?model.matrix
# now you can design your formula:
design <- model.matrix(...)
...
# Exercise 3.1.1: Create a limma object. You'll need a model.matrix,
# to function as design, you can use the information in the mew.mate dataframe
design <- model.matrix(~ class, data = new.meta)
...
Now we do have a number of differentially expressed genes (DEGs), which we can plot, subset and further categorize. 


<div style="border: 1px solid black; padding: 10px; background-color: #f9f9f9;">
<p style="text-align: center; font-weight: bold;">P-Values</p>
The p-value is a statistical measure that helps determine the significance of a hypothesis test. In the context of a moderated t-test, such as the one used in the limma package in R, the p-value represents the probability of observing the test statistic (or a more extreme value) assuming the null hypothesis is true. The null hypothesis typically states that there is no difference between the groups being compared.

The calculation of the p-value in a moderated t-test involves first computing the moderated t-statistic, which takes into account both the observed differences between the groups and the associated uncertainties. This moderated t-statistic is then compared to a t-distribution with appropriate degrees of freedom to obtain the p-value. The degrees of freedom are calculated based on the number of samples and the estimates of gene-wise variances. The p-value obtained from this process indicates the level of evidence against the null hypothesis, with smaller p-values suggesting stronger evidence in favor of the alternative hypothesis (i.e., there is a significant difference between the groups).
</div>


### Exercise 3.2 - P-values and Co
Why do we use the adjusted P-value? 
<br>Which adjustment method is used by limma?

## Strategies to handle Batch Effects

As previously mentioned, batch effects can have huge impacts on downstream analysis.
<br>Modern solutions for differential expression analysis that use linear models to model gene expression, can be fed with additional information about the batches, if these are known. (As in our case). 
However its important to note, that you can also have artifacts that are not directly correlated with the known batch! For this there are packages such as *SVA* - Surrogate variable analysis - that provide algorithms for detecting and correcting batch effects. 

We'll first use the information we have and give it to limmas model and then compare our DEGs, then we'll run SVA to see what it detects and how well it correlated with our batches. 

### Exercise 4.1 - Design matrix with confounding factors. 

To run limma with batch information, you can use your code from above almost exactly as before. However, this time we'll use the full dataset. You'll just have to add the *dataset* variable to your design formula. 
Tasks: 
a. Make one design formula with and one without the additional dataset info.
b. Run the limma analysis on the whole dataset once with each design model and save it to different result-variables. 
c. compare the outcomes: Do we have more or less DEGs?

```{r, echo=F}
# solution

# Exercise 4.1 :a)
design <- model.matrix(~ class, data = meta.data)
design_batch <- model.matrix(~ class + dataset, data = meta.data)
...
fit <- lmFit(input_cts, design)
fit <- eBayes(fit)
res_batch <- topTable(fit, number = Inf,  coef = 2)
res_batch_sig <- res %>% filter(adj.P.Val < 0.05)
```

### Exercise 4.2 - Run SVA and search for correlations with our Batch.
For this we'll need to consider a few things: <br>
a. To be able to compare our datasets/batches, we'll need to have a vector containing numbers that represents the batches. You can use  **%>% mutate()** in combination with the **ifelse()** function to generate a new column in the meta.data, called *dataset_num*.
b. SVA will take in the count data and a design matrix, so the inputs are the same as above. sva suggestst to add another design matrix mod0 for the best results, but to keep it as simple as possible we'll stay at just x=counts and mod=design. 
c. Now we have both SVs and a numerical batch representation and can use **cor()** to compute paiwise correlations of the SVs with the batch.
```{r, echo=FALSE}
# a) make a numerical version of meta.data
meta.data <- meta.data %>% 
  mutate(dataset_num = ifelse(dataset=="GSE68799", 0, 1))

# b) make a sva obj and compute surrogate variables. 
svobj <- sva(counts, design) 

# c) pairwise cors with SVs and meta.data
cor(meta.data$dataset_num, svobj$sv)
```
